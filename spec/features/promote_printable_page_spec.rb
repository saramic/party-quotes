...
describe "Promote printable page", :js do
  it "creates a promotion page with a QR code" do
    ...
    Then "there is a title, a sub title, and a QR code" do
      expect(page.find(:qa, "title").text).to eq "Matilda 18th Birthday Guest Book"
      expect(page.find(:qa, "sub-title").text).to eq "Join in the fun and upload a pic and message for Matilda's party!"
